# learn-and-write-mini-GPT-by-yourself
面向零基础学习者的 **PyTorch 迷你 GPT** 教学项目：从数据处理、预训练、问答微调（SFT）到文本生成，全程可读代码、可本地复现。
...
- 仅 CPU 也可跑通：用 `configs/tiny.yaml`
...
- `data/processed/train.txt` / `val.txt` / `test.txt`
- `data/processed/vocab.json` — 字符词表

**原理**：把 `data/raw/*.txt` 合并 → 清洗噪声行 → 按行/段随机划分，避免「前 90% 训练、后 10% 验证」造成分布不一致。
...
- `data/processed/cleaned_lines.txt` — 清洗后的单句
- `data/processed/qa.jsonl` — 结构化 `instruction / input / output`
- `data/processed/sft.jsonl` — 训练用 `问：…\n答：…` 格式
...
- 请自行准备 **有权使用** 的语料放入 `data/raw/`。
- 大文件、PDF、`checkpoints/*.pt`、`data/processed/*` 已在 `.gitignore` 中忽略，避免误传到 GitHub。
